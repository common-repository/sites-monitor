...
use WPSitesMonitor\Vendor_Prefixed\DI\Annotation\Inject;
use WPSitesMonitor\Vendor_Prefixed\DI\Annotation\Injectable;
use WPSitesMonitor\Vendor_Prefixed\DI\Definition\Exception\InvalidAnnotation;
use WPSitesMonitor\Vendor_Prefixed\DI\Definition\ObjectDefinition;
use WPSitesMonitor\Vendor_Prefixed\DI\Definition\ObjectDefinition\MethodInjection;
use WPSitesMonitor\Vendor_Prefixed\DI\Definition\ObjectDefinition\PropertyInjection;
use WPSitesMonitor\Vendor_Prefixed\DI\Definition\Reference;
use WPSitesMonitor\Vendor_Prefixed\Doctrine\Common\Annotations\AnnotationRegistry;
use WPSitesMonitor\Vendor_Prefixed\Doctrine\Common\Annotations\Reader;
use WPSitesMonitor\Vendor_Prefixed\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use WPSitesMonitor\Vendor_Prefixed\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'WPSitesMonitor\Vendor_Prefixed\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'WPSitesMonitor\Vendor_Prefixed\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('WPSitesMonitor\Vendor_Prefixed\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()
                ->getClassAnnotation($class, 'WPSitesMonitor\Vendor_Prefixed\DI\Annotation\Injectable');
